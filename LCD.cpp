...
// custom definitions
// TODO: swap hard-coded magic numbers
#define CMD_INSTR_SET 0x20
#define EXTEND_INSTR 0x01
#define NORMAL_PRINT 0x0C
#define INVERSE_PRINT 0x0D
#define SET_CONTRAST 0xB0
#define LCD_BIAS_MODE 0x14
#define SET_TEMP 0x04
...
void LCD::init_lcd(void) {
	...
	send_cmd(CMD_INSTR_SET | EXTEND_INSTR);
//	send_cmd(0xB0);
	send_cmd(SET_CONTRAST | 0x0F); // bit or for level
	send_cmd(SET_TEMP);
	send_cmd(LCD_BIAS_MODE);
	// H = 0
	send_cmd(CMD_INSTR_SET);
	send_cmd(NORMAL_PRINT);
